build_thredds_netcdf_6 <- function(buoys = "list of buoys", start_year = "2021", end_year = "2022"){
     ...
     if (!file.exists(paste0(data_dir,"best_netCDF_year_mth/"))){dir.create((paste0(data_dir,"best_netCDF_year_mth/")))}
     if (!file.exists(paste0(data_dir,"best_netCDF_local/"))){dir.create((paste0(data_dir,"best_netCDF_local/")))}
     best_netCDF_dir <- paste0(data_dir,"best_netCDF_local/")
     best_netCDF_year_mthdir <- paste0(data_dir,"best_netCDF_year_mth/")
     ...
     # buoys <- c("41001", "41002", "41004", "41006", "41008", "41009", "41010", "41012", "41013", "41018",
     #            "41021", "41022", "41023", "41025", "41035", "41036", "41040", "41041", "41043", "41044",
     #            "41046", "41047", "41048", "41049",
     #            "42001", "42002", "42003", "42007", "42012", "42019",
     #            "42020", "42035", "42036", "42038", "42039", "42040", "42041", "42053", "42054", "42055",
     #            "42056", "42057", "42058", "42059", "42060", "42065", "42080", #"42042",
     #            "44004", "44005", "44007", "44008", "44009", "44011", "44013", "44014", "44017", "44018", 
     #            "44020", "44025", "44027", "44028", "44065", "44066", "44070",
     #            "45001", "45002", "45003", "45004", "45005", "45006", "45007", "45008", "45011", "45012", 
     #            "46001", "46002", "46003", "46005", "46006", "46011", "46012", "46013", "46015", "46022", 
     #            "46023", "46025", "46026", "46027", "46028", "46029", "46030", "46035", "46041", "46042", 
     #            "46045", "46047", "46050", "46051", "46053", "46054", "46059", "46060", "46061", "46062", 
     #            "46063", "46066", "46069", "46070", "46071", "46072", "46073", "46075", "46076", "46077", 
     #            "46078", "46079", "46080", "46081", "46082", "46083", "46084", "46085", "46086", "46087", 
     #            "46088", "46089", "46105", "46106", "46107", "48011", 
     #            "51000", "51001", "51002", "51003", "51004", "51026", "51028", "51100", "51101", "52009")
     print(buoys)
     ...
     for(buoy in buoys[82:length(buoys)]){
          # buoy <- buoys[1]
          Sys.time()
          ...
          # load buoy data
          if(file.exists(paste0(input_dir,buoy,"/s_",buoy,"_best_data.RData"))){
              ...
              date_year_start <- as.integer(start_year)
              date_year_end <- as.integer(end_year)
              rm(dat)
              ...
              for(dateRange in date_range){
                  ...
                  for(m in monthRange){
                      ...
                      # if (!file.exists(paste0(best_netCDF_year_mthdir,buoy,"/"))){dir.create((paste0(best_netCDF_year_mthdir,buoy,"/")))}
                      # if (!file.exists(paste0(best_netCDF_year_mthdir,buoy,"/",dateRange,"/"))){dir.create((paste0(best_netCDF_year_mthdir,buoy,"/",dateRange,"/")))}
                      # year_mth_dir <- paste0(best_netCDF_year_mthdir,buoy,"/",dateRange,"/")
                      # buoy_dir <- paste0(best_netCDF_dir,buoy,"/")
                      # 
                      ...
                      if(sum(str_count(dat_ls, "_ndbc_stdmet"))==0){
                      ...
                      if(length(dat_ls)>0){
                          ...
                          met_ls <- ls(pattern = paste0(dateRange,"_",m,"_s_",buoy,"_ncei_station_metadata_"))
                          if(length(met_ls)>0){
                               depth_df <- get(ls(pattern = paste0(dateRange,"_",m,"_s_",buoy,"_ncei_station_metadata_")))
                               depth_glob <- median(depth_df$depth, na.rm = TRUE)
                               lat_max_glob <- round(max(depth_df$lat, na.rm = TRUE),2)
                               lat_min_glob <- round(min(depth_df$lat, na.rm = TRUE),2)
                               lon_max_glob <- round(max(depth_df$lon, na.rm = TRUE),2)
                               lon_min_glob <- round(min(depth_df$lon, na.rm = TRUE),2)
                               rm(depth_df)
                          }else{
                              depth_df <- get(ls(pattern = paste0("s_",buoy,"_ncei_station_metadata_")))
                              depth_df <-dplyr::filter(depth_df, year(DateTime) == as.numeric(dateRange))
                              depth_glob <- median(depth_df$depth, na.rm = TRUE)
                              lat_max_glob <- round(max(depth_df$lat, na.rm = TRUE),2)
                              lat_min_glob <- round(min(depth_df$lat, na.rm = TRUE),2)
                              lon_max_glob <- round(max(depth_df$lon, na.rm = TRUE),2)
                          ...
                          if(length(freq_ls)>0){
                              ...
                              # # adding 0.0100 and 0.0200 columns to old freq if necessary
                              # old_freq <- freq_ls[grep("_old", freq_ls)]
                              # for(df1 in old_freq){
                              #     df <- get(df1)
                              #     if(ncol(df)!=wave_old_length+3){
                              #         if("0.0100" %in% names(df)==FALSE){df$`0.0100`<-NA}
                              #         if("0.0200" %in% names(df)==FALSE){df$`0.0200`<-NA}
                              #         df <- dplyr::select(df, c("DateTime", "0.0100","0.0200","0.0300","0.0400","0.0500","0.0600","0.0700","0.0800","0.0900",
                              ...
                              #                                   "0.3000","0.3100","0.3200","0.3300","0.3400","0.3500","0.3600","0.3700","0.3800","0.3900","0.4000"))
                              #         assign(df1,df)
                              #     }
                              #     rm(df)
                              # }
                          ...
                          for(var_df in var_ls){
                              ...
                              if(var_df == "lat") {var_name = "latitude"; var_units = 'degrees_north'; var_longname='latitude'; variable_prec= variable_prec_df}
                              if(var_df == "lon") {var_name = "longitude"; var_units = 'degrees_east'; var_longname='longitude'; variable_prec= variable_prec_df}
                          ...
                          ncatt_put(nc=con,varid=0, attname="geospatial_conventions", attval = "Positions are presented in decimal degrees with latitudes north of the equator represented as positive values and longitudes west of the Prime Meridian represented as negative values", prec="char")
                          ...
                          ncatt_put(nc=con,varid=0, attname="creator_name", attval = "National Data Buoy Center", prec="char")
                          ncatt_put(nc=con,varid=0, attname="creator_url", attval = "https://www.ndbc.noaa.gov/)", prec="char")
          ...
          }else{print(paste0("no new data for buoy ", buoy))}
