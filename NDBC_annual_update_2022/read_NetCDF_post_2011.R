read_NetCDF_post_2011 <- function(x = "list of files", buoy = "buoy number", ascii_ncei_dir = "ascii_ncei_dir"){
        ...
        tryCatch({
                ...
                if(nc$nvars != 0){
                        ...
                        if("gps_1/longitude" %in% dat_list){
                        ...
                        for(j in dat_use){
                                # j <- dat_use[2]
                                # j <- nc$var$`anemometer_1/wind_direction`$dim[[1]]$vals
                                # print(j)
                                dat <- data.frame(ncvar_get(nc, j), stringsAsFactors = FALSE)
                                # df <- data.frame(nc$var$`anemometer_1/wind_direction`$dim[[1]]$vals, stringsAsFactors = FALSE)
