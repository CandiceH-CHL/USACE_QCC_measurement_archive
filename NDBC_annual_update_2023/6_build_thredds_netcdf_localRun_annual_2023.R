build_thredds_netcdf_6 <- function(buoys = "list of buoys", start_year = "2021", end_year = "2022"){
     ...
     drive <- "D:/"
     # main dir
     data_dir <- paste0(drive, "Candice/projects/WaveTrends/NDBC_2024/data/")
     setwd(data_dir)
     # set input directories
     input_dir <- paste0(data_dir,"best_data/")
     # set new output directories for datasets
     if (!file.exists(paste0(data_dir,"best_netCDF/"))){dir.create((paste0(data_dir,"best_netCDF/")))}
     best_netCDF_dir <- paste0(data_dir,"best_netCDF/")
     ...
     buoys <- list.dirs(input_dir)
     buoys <- gsub(input_dir,'',buoys)
     buoys <- gsub('/','',buoys)
     buoys <- buoys[nzchar(buoys)]
     print(buoys)
     ...
     for(buoy in buoys){
          ...
          if(file.exists(paste0(input_dir,buoy,"/s_",buoy,"_best_data.RData"))){
              ...
              # find date range
              # load stdmet data
              dat <- get(data_ls[data_ls %like% 'ndbc_stdmet_geoClean'])
              ...
              for(dateRange in date_range){
                  ...
                  for(m in monthRange){
                      ...
                      for(df in data_ls){
                          ...
                          new_name <- gsub('_geoClean','',new_name)
                          if(dim(dat)[2]==50){new_name <- paste0(new_name,"_freq")}
                      ...
                      if(length(dat_ls)>0){
                          ...
                          year_mth_dir <- paste0(best_netCDF_dir,buoy,"/",dateRange,"/")
                          if(!file.exists(year_mth_dir)){dir.create(year_mth_dir)}
                          ...
                          depth_glob <- median(dat$depth, na.rm = TRUE)
                          lat_max_glob <- round(max(dat$lat, na.rm = TRUE),2)
                          lat_min_glob <- round(min(dat$lat, na.rm = TRUE),2)
                          lon_max_glob <- round(max(dat$lon, na.rm = TRUE),2)
                          lon_min_glob <- round(min(dat$lon, na.rm = TRUE),2)
                          ...
                          if(sum(str_count(dat_ls, "_freq"))>0){
                              for(df in dat_ls){
                                  # df <- dat_ls[1]
                                  df_name <- paste0("dat_spec_",unlist(strsplit(df,paste0("_",buoy,"_")))[2])
                                  dat_spec <- get(df)
                          ...
                          dat_ls <- dat_ls[!grepl('_freq', dat_ls)]
                          # housekeeping
                          if(length(dat_ls)==0){
                          ...
                          data.time <- as.numeric(dat$DateTime)
                          tunits1 <- "seconds since 1970-01-01 00:00:00"
                          ...
                          if(length(freq_ls)>0){
                              ...
                              dat_spec <- get(freq_ls[grep("_freq",freq_ls)[1]])
                          ...
                          for(var_df in var_ls){
                              ...
                              if(var_df == "mooring"|var_df == "hull"|var_df == "payload"| var_df == 'waveSys_Sensor'){variable_prec = variable_prec_metadata; var_units = ''; rm(miss_values); var_longname = paste0("NDBC ",var_df, " type")}
                              ...
                              # set frequency bands
                              if(grepl("_freq", var_df)){var_name = paste0(var_name,"_",wave_new_length, "Frequencies")}
                              ...
                              if(grepl("_freq", var_df)){
                                  ...
                                  # create matrix
                                  mat_df <- matrix(t(dat_spec), nrow=ncol(dat_spec),ncol=nrow(dat_spec))
                              ...
                              }else if(var_df == "mooring"|var_df == "hull"|var_df == "payload"| var_df == 'waveSys_Sensor'){
                                  vari_df <- ncvar_def(name=var_name, units=var_units, longname=var_longname, dim=list(dimTime,dimnchar), prec=variable_prec)
                          ...
                          for(var_df in var_list){
                              ...
                              if(std_name == "waveSysSensor"){var_standard = ''; desc_name = "NDBC Verified Wave Sensor"}
                          ...
                          ncatt_put(nc=con,varid=0, attname="geospatial_lat_units", attval = "degrees north", prec="char")
                          ...
                          ncatt_put(nc=con,varid=0, attname="geospatial_lon_units", attval = "degrees east", prec="char")
                          ...
                          ncatt_put(nc=con,varid=0, attname="depth_units", attval = "meters below mean sea level", prec="char")
                          ...
                          ncatt_put(nc=con,varid=0, attname="time_format", attval = 'UNIX EPOCH', prec="char")
                          ...
                          ncatt_put(nc=con,varid=0, attname="publisher_name", attval = "U.S. Army Corps of Engineers (USACE) Engineer Research and Development Center (ERDC) Coastal Ocean Data Systems (CODS) Program", prec="char")
